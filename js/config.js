/**
 * ==================================================================================
 * 模块名称：ConfigModule (全局静态配置中�?
 * 目标文件：js/config.js
 * 
 * 【设计哲学】：
 * ConfigModule 是全站的“法典”。它不包含一行可执行逻辑，但它定义的每一个数�? * 和字符串都决定了系统的运行规则。将配置硬隔离在此处，是为了确保当项目迁移到
 * 生产环境或更换数据库时，我们只需修改这一个文件即可完成全站适配�? * 
 * 【配置项清单 & 业务初衷】：
 * 
 * 1. firebaseConfig [连接凭据]
 *    - 【包含】：apiKey, authDomain, databaseURL �?7 项核心参数�? *    - 【存在理由】：这是全站�?Google Cloud 服务握手的唯一钥匙�? * 
 * 2. APP_CONSTANTS [业务常量]
 *    - 【ADMIN_EMAIL】：Moss 的官方管理邮箱，是判定最高权限的硬编码依据�? *    - 【BOT_IDs】：定义�?Safety Bot �?Advice Bot �?ID。这�?ID 在数据库中是特殊的，系统会自动跳过它们的真实档案拉取逻辑�? *    - 【SVG 资产】：�?Eagle Icon 等核心视觉资�?Base64 化，减少网络请求，实现秒开体验�? * 
 * 3. LIMITS [反垃圾系统]
 *    - 【MSG_COOLDOWN (1000ms)】：防止用户通过脚本疯狂刷屏导致 Firebase 计费爆炸�? *    - 【POST_COOLDOWN (5000ms)】：发帖冷却�? *    - 【存在理由】：这是最简单的“软件防火墙”，在前端拦截恶意请求�? * 
 * 4. SYSTEM_USERS [虚拟账户池]
 *    - 【存在理由】：当系统发送自动报警或建议时，会显示为这些虚拟账户。将它们的头像和名称统一定义在这里，方便未来统一进行品牌升级�? * ==================================================================================
 */

// 不能涉及到任何跟module相关的变量，因为后续module需要进行剥�?export const firebaseConfig = {
...
// 初始化的系统用户资料（虚拟账户与官方账号�?export const SYSTEM_USERS = {
    'safety_bot': {
        id: 'safety_bot',
        name: 'Safety Bot',
    ...
    'moss104088': {
        id: 'moss104088',
        name: 'Moss',
        avatar: `data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'%3E%3Cpath fill='%23FFF' d='M20 60c0-10 10-20 30-20s30 10 30 20-10 20-30 20-30-10-30-20z'/%3E%3Cpath fill='%23FF7F50' d='M15 55c0-15 20-30 35-30s35 15 35 30c0 5-10 10-35 10S15 60 15 55z'/%3E%3Cpath fill='none' stroke='%23FFF' stroke-width='1.5' d='M30 35c10 5 30 5 40 0M25 45c10 5 35 5 45 0M20 55c10 5 40 5 50 0'/%3E%3Cpath fill='%23333' d='M45 25c2 2 8 2 10 0l-2-3-6 1z'/%3E%3C/svg%3E`,
        email: 'moss104088@gmail.com',
        role: 'admin'
    }
};
