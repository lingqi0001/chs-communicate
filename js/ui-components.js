...
import { WritingDocCard } from './writing.js';
...
export const UIComponents = {
    ...
    createChatBubble: function (msg, key, currentUser, setupLongPressCallback, showSenderName = true) {
        ...
        // Project soft-filter anchor: which writing doc this message belongs to.
        const docLinkMatch = (msg.text || '').match(/docs\.google\.com\/document\/d\/([a-zA-Z0-9-_]+)/);
        let msgDocId = (msg.docData && msg.docData.fileId)
            || (docLinkMatch ? docLinkMatch[1] : '')
            || (msg.type === 'comment_card' && msg.commentCard ? msg.commentCard.docId : '');
        
        // 🆕 NEW: If no single docId but has projectDocIds, use comma-separated list
        if (!msgDocId && msg.projectDocIds && msg.projectDocIds.length > 0) {
            msgDocId = msg.projectDocIds.join(',');
        }
        
        if (msgDocId) div.setAttribute('data-doc-id', msgDocId);
        // Anchor for portfolio comment Locate: the chat message that carried
        // this Google comment when it was synced/pushed into the conversation.
        if (msg.type === 'comment_card' && msg.commentCard && msg.commentCard.comment) {
            const anchors = [msg.commentCard.comment.id, ...(msg.commentCard.anchorIds || [])].filter(Boolean).join(' ');
            if (anchors) div.setAttribute('data-comment-id', anchors);
        }
        ...
        if (msg.type === 'comment_card' && msg.commentCard) {
            ...
            if (window.isPhotoDisabled) {
                ...
                content = UIComponents.renderImageMedia(images, key, isMe, msg.isExpired);
            }
        ...
        if (msg.quote) {
            ...
            qDiv.className = `text-[11px] mt-0.5 px-2.5 py-1 max-w-[78%] border border-gray-200/80 dark:border-white/10 bg-white dark:bg-[#1C1C1E] text-gray-500 dark:text-gray-400 shadow-sm ${cornerClass} text-left truncate whitespace-nowrap`;
    ...
    /**
     * Shared image media renderer: a single rounded card for one photo, or
     * the fanned stack + "Expand N" pill for several. Used by regular image
     * messages and comment-card forwarded photos so both look identical.
     */
    renderImageMedia: function (images, key, isMe, isExpired) {
        const enc = encodeURIComponent(JSON.stringify(images));
        if (images.length === 1) {
            return `<div class="relative w-36 h-48 bg-gray-100 dark:bg-[#2C2C2E] rounded-2xl overflow-hidden border border-gray-200 dark:border-white/5 shadow-sm">
                        <img src="${UIUtils.escape(images[0])}" 
                             onerror="this.style.display='none'; this.nextElementSibling.classList.remove('hidden');" 
                             onclick="openGallery('${enc}', 0)" 
                             class="w-full h-full object-cover cursor-pointer ${isExpired ? 'opacity-20 grayscale' : ''}">
        ...
        for (let i = images.length - 1; i >= 0; i--) {
            ...
            sH += `<div id="img-${key}-${i}" class="absolute left-0 w-36 h-48 bg-gray-100 dark:bg-[#2C2C2E] rounded-2xl shadow-md transition-all duration-300 ${isMe ? 'origin-left' : 'origin-right'} cursor-pointer overflow-hidden border border-gray-200 dark:border-white/5"
                        ...
                        <img src="${UIUtils.escape(images[i])}" 
                             onerror="this.style.display='none'; this.nextElementSibling.classList.remove('hidden');"
                             class="w-full h-full object-cover ${isExpired ? 'opacity-20 grayscale' : ''}">
                        <div class="hidden absolute inset-0 flex flex-col items-center justify-center text-gray-400 p-4 text-center">
        ...
        return `<div id="stack-${key}" class="relative w-36 h-48 ${isMe ? 'ml-24 mr-8' : 'mr-24 ml-8'} mb-2 mt-1 transition-all duration-300">
    ...
    /**
     * [Comment Card Message] 单条 Google Doc 评论被转发后在聊天中生成的评论。
     * 渲染为普通消息气泡(评论正文) + 下方一条 reply 样式上下文条。
     */
    renderCommentCardMsg: function (msg, key, isMe) {
        ...
        const cPhoto = UIComponents.splitPhoto(cDisp.content || '');
        const content = cPhoto.text.trim();
        window._commentCards = window._commentCards || {};
        window._commentCards[key] = card;

        const bS = isMe
            ? 'chat-accent-bg bg-[#007AFF] text-white rounded-3xl rounded-br-sm'
            : 'other-accent-bg bg-[#E9E9EB] dark:bg-gray-700 text-black dark:text-white rounded-3xl rounded-bl-sm';
        const bubble = content
            ? `<div class="px-[18px] py-2 text-base leading-[1.4] max-w-[75%] inline-block break-words whitespace-pre-wrap shadow-sm ${bS}">${UIUtils.linkify(UIUtils.escape(content), isMe)}</div>`
            : '';
        const photoHtml = cPhoto.photoUrls.length ? UIComponents.renderImageMedia(cPhoto.photoUrls, `${key}-cc`, isMe, false) : '';

        const cornerClass = isMe
            ? 'rounded-tl-xl rounded-bl-xl rounded-br-xl rounded-tr-none'
            : 'rounded-tr-xl rounded-br-xl rounded-bl-xl rounded-tl-none';
        const originMsgKey = card.originMsgKey || '';
        const sourceDocUrl = card.docUrl || '';
        const canJump = !!(originMsgKey || sourceDocUrl);
        const jumpAttrs = canJump
            ? `onclick="window.jumpCommentCard(this)" data-origin-key="${UIUtils.escape(originMsgKey)}" data-doc-url="${UIUtils.escape(sourceDocUrl)}" data-comment-id="${UIUtils.escape(c.id || '')}"`
            : '';
        const textCls = canJump ? ' cursor-pointer hover:underline' : '';
        const textGroup = `<div class="flex-1 min-w-0"><span ${jumpAttrs} class="text-[10px] font-bold uppercase tracking-wide text-[#007AFF] dark:text-[#0A84FF]${textCls}">${UIUtils.escape(author)} Commented on ${UIUtils.escape(docTitle)}</span></div>`;
        const docBtn = `<button type="button" onclick="window.replyCommentCardStrip('${key}', event)" class="w-6 h-6 flex-shrink-0 rounded-full hover:bg-[#007AFF]/10 dark:hover:bg-[#007AFF]/25 flex items-center justify-center text-black dark:text-white transition-colors" title="Reply to this comment"><svg class="w-3 h-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3 10h10a8 8 0 0 1 8 8v2M3 10l6 6m-6-6l6-6"/></svg></button>`;
        const expandBtn = `<button type="button" onclick="window.toggleCommentCardExpand('${key}', event)" class="w-6 h-6 flex-shrink-0 rounded-full hover:bg-[#007AFF]/10 dark:hover:bg-[#007AFF]/25 flex items-center justify-center text-black dark:text-white transition-colors" title="Show comment thread"><svg id="ccArrow-${key}" class="w-3.5 h-3.5 transition-transform duration-300 ease-[cubic-bezier(0.22,1,0.36,1)]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"></polyline></svg></button>`;
        // docBtn sits on the non-edge side: left for own (right-aligned) messages,
        // right for others' (left-aligned) messages.
        const strip = `<div class="flex items-center gap-1 text-[11px] mt-0.5 px-2.5 py-1 max-w-[75%] border border-gray-200/80 dark:border-white/10 bg-white dark:bg-[#1C1C1E] shadow-sm ${cornerClass} text-left break-words">${isMe ? docBtn + textGroup + expandBtn : expandBtn + textGroup + docBtn}</div>`;
        const drawer = `<div id="ccDrawer-${key}" class="doc-drawer-accordion hidden w-full mt-1">
            <div class="min-h-0 overflow-hidden">
                <div class="bg-white dark:bg-[#1C1C1E] border border-gray-200/80 dark:border-white/10 shadow-sm rounded-2xl px-3 py-1 text-left">
                    <div id="ccList-${key}"></div>
                </div>
            </div>
        </div>`;

        return `<div class="flex flex-col ${isMe ? 'items-end' : 'items-start'} w-full max-w-full mb-2">${bubble}${photoHtml}${strip}${drawer}</div>`;
    }
};

// Doc card implementation lives in writing.js; merged back here so the
// existing UIComponents.* call sites (chat.js / search.js / portfolio) keep working.
Object.assign(UIComponents, WritingDocCard);
